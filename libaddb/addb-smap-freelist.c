...
int addb_smap_freelist_alloc(addb_smap_partition *part, size_t ex,
                             unsigned long long *offset_out) {
  addb_smap *sm = part->part_sm;
  addb_handle *ad = sm->sm_addb;
  cl_handle *cl = ad->addb_cl;
  ...
  err = addb_smap_partition_get(part, slot_offset, &slot_val);
  if (err) return err;
  ...
  *offset_out = off;
   ...
   *  slot := pointer;
   */
  err = addb_smap_partition_get(part, off, &pointer);
  if (err != 0) return err;

  err = addb_smap_partition_put(part, slot_offset, pointer);
  if (err != 0) return err;
 ...
 */
int addb_smap_freelist_free(addb_smap_partition *part, unsigned long long off,
                            size_t ex) {
  int err;
  ...
  /*
  cl_log(cl, CL_LEVEL_SPEW, "smap part %s: freelist(%d) += %llu[%llu] "
          "(at %p)",
  ...
  */
   ...
   *  obj->next = head;
   */
  err = addb_smap_partition_get(part, head_offset, &head_val);
  if (err != 0) return err;

  err = addb_smap_partition_put(part, off, head_val);
  if (err != 0) return err;
  ...
  cl_assert(part->part_sm->sm_addb->addb_cl, ull != 0);

  err = addb_smap_partition_put(part, head_offset, ull);
  if (err != 0) return err;
